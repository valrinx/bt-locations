...
// ════════════════════════════════════════════
// BUILD STATUS BADGE (Supabase Realtime)
// ════════════════════════════════════════════
let buildStatusChannel = null;
let _lastBuildStatus = null;

function initBuildStatusRealtime() {
    // Subscribe to build_status table changes
    buildStatusChannel = _sb
        .channel('build_status_changes')
        .on('postgres_changes',
            { event: '*', schema: 'public', table: 'build_status' },
            (payload) => {
                console.log('[Build Status] Realtime:', payload);
                updateBuildBadge(payload.new);
            }
        )
        .subscribe((status) => {
            console.log('[Build Status] Subscription:', status);
        });
    
    // Fetch latest status on init
    fetchLatestBuildStatus();
}

async function fetchLatestBuildStatus() {
    ...
    if (error) {
        console.log('[Build Status] No previous build found');
        return;
    }
    
    if (data) {
        _lastBuildStatus = data;
        updateBuildBadge(data);
    }
}

function updateBuildBadge(status) {
    if (!status) return;
    ...
    if (!badge) {
        ...
        // Insert after sync badge or in top-right area
        const syncBadge = document.getElementById('syncBadge');
        if (syncBadge && syncBadge.parentNode) {
    ...
    if (status.status === 'pending') {
        badge.innerHTML = '🔄 กำลัง build...';
        badge.className = 'build-badge pending show';
        showToast('🔄 GitHub Actions กำลัง build...', false, true);
    } else if (status.status === 'success') {
        const ago = timeAgo(status.completed_at || status.started_at);
        badge.innerHTML = `✅ Build สำเร็จ ${ago}`;
        badge.className = 'build-badge success show';
        showToast('✅ Build สำเร็จแล้ว!', false, true);
        ...
        badge.innerHTML = '❌ Build ล้มเหลว';
        badge.className = 'build-badge failed show';
        showToast('❌ Build ล้มเหลว กรุณาตรวจสอบ', true, true);
    }
}
...
// ════════════════════════════════════════════
// BUILD STATUS CSS (injected)
// ════════════════════════════════════════════
(function injectBuildStatusCSS() {
    ...
    css.textContent = `
        ...
        @keyframes buildPulse {
            ...
            50% { opacity: 0.85; box-shadow: 0 0 0 8px rgba(245, 166, 35, 0); }
        }
    `;
...
if (typeof _sb !== 'undefined' && _sb) {
    ...
    setTimeout(initBuildStatusRealtime, 2000);
}